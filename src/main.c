...
/*   Updated: 2023/08/22 14:36:53 by ekoljone         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_player(t_cubed *cubed)
{
	...
	x = cubed->player.px;
	y = cubed->player.py;
	while(x < (cubed->player.px + 4))
	{
		while (y < (cubed->player.py + 4))
		{
...
{
	 int minimapSize = 4; // Adjust this to your desired size

    // Calculate the scaling factor
    float scaleFactor;
    int mapWidth =	cubed->map.mapX; // Replace with your map's width
    int mapHeight = cubed->map.mapY; // Replace with your map's height

    if (mapWidth >= mapHeight) {
        scaleFactor = (float)minimapSize / mapWidth;
    } else {
        scaleFactor = (float)minimapSize / mapHeight;
    }
    // Apply the scaling factor to the minimap dimensions
    int minimapWidth = (int)(mapWidth * scaleFactor);
    int minimapHeight = (int)(mapHeight * scaleFactor);
	cubed->map.mapS = minimapWidth * minimapHeight;
